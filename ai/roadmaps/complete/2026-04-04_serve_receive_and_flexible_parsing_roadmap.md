...
- Scope status: Closed after the schema addition, deterministic parser expansion, and repo-local verification documented in [`aiDocs/evidence/phase_6_serve_receive_and_flexible_parsing_verification.md`](../../aiDocs/evidence/phase_6_serve_receive_and_flexible_parsing_verification.md).
...
- [x] **Schema support**: add `serve_receive` to the stat-event enum/type flow so the DB, typed client, stats aggregation, and UI all recognize it as a normal confirmed event.
- [x] **Live stats/report support**: surface serve receive anywhere core stat totals, player rows, summaries, and post-game generation depend on the stat-event vocabulary.
- [x] **Deterministic parser expansion**: support more volleyball-natural aliases and relation patterns around passing/serve receive, setting language, and longer spoken sentence structure.
- [x] **Multi-event integration**: allow mixed rally recaps like “Chuck serve receive, passed to Shane, Shane set, Cosmo kill” to produce one grouped review item with supported rows plus skipped context where appropriate.
- [x] **Clause-level AI compatibility**: preserve the current rule that AI only assists unresolved eligible clauses, now against the expanded vocabulary and broader clause shapes.
- [x] **Verification**: record parser examples, serve receive persistence/report checks, and mixed deterministic/AI recovery notes in an evidence doc before archiving.

## Readiness Checks

- [x] Serve receive is treated as a normal supported stat event end to end, not as metadata or notes.
- [x] Existing confirmed-event trust flows remain unchanged.
- [x] The parser broadens for volleyball-natural phrasing without turning every long sentence into an AI-first path.
- [x] Mixed rally recaps still stay grouped in one review card.
