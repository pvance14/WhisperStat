...
Maps directly to Postgres tables (names indicative):

- **teams** — `id`, `name`, owner `user_id` / org linkage.
- **players** — `id`, `team_id`, name, jersey, optional `aliases[]` for NLP.
- **games** — `id`, `team_id`, opponent label, set/score fields, metadata.
- **stat_events** — `id`, `game_id`, `player_id`, action, `set`, `occurred_at`, source (`voice` / `manual`), optional `raw_transcript` (privacy policy TBD).
- **game_summaries** — `game_id`, body text, `generated_at`, optional `model` / version.

Use FKs and indexes on `(game_id, set)`, `(game_id, player_id)` for dashboard queries.
